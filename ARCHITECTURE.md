...
- PowerPC: the reusable 750/Gekko core executes GameCube and Wii-class 32-bit PowerPC state, including paired-single arithmetic, comparisons, merge operations, PSQ quantized loads/stores, GQR0-7 and HID2 controls, table-driven `fres`/`frsqrte` plus paired estimate variants, full-width single-precision FPR operands, Gekko 25-bit frC multiply/FMA handling, fused scalar/paired FMA rounding, FPSCR rounding/NI behavior, FPRF/FPCC classification, NaN-aware compare behavior, invalid arithmetic status, and correct opcode-63 A-form decode. Remaining CPU fidelity work is concentrated in overflow/underflow/inexact status edges and extreme subnormal/FMA exponent cases. Wii U reuses the same integer execution family for Espresso development paths. A separate reusable 64-bit big-endian PowerPC substrate serves Xbox 360 Xenon and PlayStation 3 PPE development execution.
