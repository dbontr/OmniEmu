...
PlayStation is the first fifth-generation machine graph. It uses the reusable MIPS R3000A execution core with mirrored system RAM, scratchpad and a user-provided 512 KiB BIOS, routes COP0 external interrupts through the console interrupt controller, implements GPU VRAM/display and an initial command rasterizer, GPU block/linked-list plus OTC DMA, timer/SPU register foundations, digital-pad serial I/O, memory-card persistence and deterministic state. CD-ROM media, SPU synthesis, textured/blended GPU commands and cycle-level timing remain foundation work.
