...
- PowerPC: the reusable 750/Gekko core executes GameCube and Wii-class 32-bit PowerPC state, including paired-single arithmetic, comparisons, merge operations, PSQ quantized loads/stores, GQR0-7 and HID2 controls, table-driven `fres`/`frsqrte` plus paired estimate variants, full-width single-precision FPR operands, Gekko 25-bit frC multiply/FMA handling, fused scalar/paired FMA rounding, FPSCR rounding/NI behavior, disabled single-precision overflow/underflow/inexact status with FI/FR/XX, FPRF/FPCC classification, NaN-aware compare behavior, invalid arithmetic status, and correct opcode-63 A-form decode. Remaining CPU fidelity work is concentrated in enabled overflow/underflow/inexact exception-result delivery, paired-single rounding-flag aggregation, and pathological subnormal/FMA exponent cases. Wii U reuses the same integer execution family for Espresso development paths. A separate reusable 64-bit big-endian PowerPC substrate serves Xbox 360 Xenon and PlayStation 3 PPE development execution.
...
GameCube is the third sixth-generation proof point and the first PowerPC machine. The Gekko substrate extends the reusable 750-class interpreter with paired-single/GQR/HID2 state, table-driven scalar/paired reciprocal and reciprocal-square-root estimates, full-width single-precision operands, Gekko 25-bit frC multiply/FMA handling, fused scalar/paired FMA rounding, FPSCR rounding/NI behavior, disabled single-precision overflow/underflow/inexact status with FI/FR/XX, FPRF/FPCC classification, NaN-aware compare semantics, invalid arithmetic status and corrected opcode-63 A-form decoding; the machine maps 24 MiB MEM1, 16 MiB ARAM and Flipper PI/DI/SI/VI/DSP register foundations, consumes the GX write-gather stream through CP/BP/XF/indexed register loads, VCD/VAT-aware primitive framing and bounded nested display-list traversal, decodes direct/indexed position, color and TexCoord0 data, applies position matrices plus orthographic/perspective XF projection and viewport/scissor mapping, performs perspective-correct TexCoord0 interpolation, selects point or linear base-level texture-unit-0 filtering from local LOD derivatives and BP minification/magnification state, samples I4/I8/IA4/IA8/RGB565/RGB5A3/RGBA8/C4/C8/C14X2/CMPR formats with hardware-style clamp/repeat/mirror wrapping and BP-driven TMEM/TLUT palette loading for IA8/RGB565/RGB5A3 palettes, rasterizes color- and texture-modulated primitives into the EFB with projected-face culling, 24-bit depth compare/write, BP blend/logic operations, color/alpha write masks, constant alpha and dither, clears color/depth through the BP copy path, and executes BP-driven YUYV EFB-to-XFB copies into MEM1 for VI presentation, HLE-loads DOL sections from raw executable or disc data, reconstructs BS2 Disc-ID/DTK boot state and streams DVD DMA through the common sparse resource layer with Disc-ID gating, packed Request Error state, seek/stop-motor handling, DTK configuration/status commands, demand-paged 48 kHz stereo DTK ADPCM decoding/mixing and block-timed stream position, translates GameCube controller state through SI, presents YUYV XFB output, produces audio-DMA samples, and serializes deterministic state. EXI channels 0 and 1 expose independent Slot A and Slot B memory cards with transfer-complete/device interrupts, DMA, guest-visible read/program/erase behavior and persistent backing. EXI channel 2 exposes the AD16 diagnostics device with ID and 32-bit trace-register transfers. Channel 0 also exposes the IPL device, including optional user-supplied 2 MiB IPL dumps, raw boot-code descrambling, embedded font reads, UART/eUART debug-register handling, SRAM access and deterministic RTC progression. Remaining Gekko CPU fidelity is concentrated in enabled overflow/underflow/inexact exception-result delivery, paired-single rounding-flag aggregation, and pathological subnormal/FMA exponent cases; full GX/Flipper graphics, DSP microcode, additional EXI devices, sub-frame DTK scheduling and exact bus/timing behavior remain machine-specific work.
