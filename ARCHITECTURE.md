...
- PowerPC: the reusable 750/Gekko core executes GameCube and Wii-class 32-bit PowerPC state, including paired-single arithmetic, comparisons, merge operations, PSQ quantized loads/stores, GQR0-7 and HID2 controls, and table-driven `fres`/`frsqrte` plus paired estimate variants with signed-zero, infinity and NaN handling. Wii U reuses the same integer execution family for Espresso development paths. A separate reusable 64-bit big-endian PowerPC substrate serves Xbox 360 Xenon and PlayStation 3 PPE development execution.
