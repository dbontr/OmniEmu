...
- Reusable Sony SPC700 execution engine with all 256 opcode decode paths, direct-page/bit operations, 8/16-bit arithmetic, branches/calls/stack flow, multiply/divide/decimal adjust, and deterministic state serialization; S-SMP RAM, ports and timers are integrated into the SNES machine with a firmware-free IPL transfer HLE. The S-DSP foundation now decodes BRR blocks and mixes eight stereo voices at 32 kHz with pitch, key on/off, direct/basic ADSR/GAIN envelopes, noise and ENDX state; Gaussian interpolation, exact envelope rates, pitch modulation and echo/FIR remain.
...
- Runnable Genesis / Mega Drive development machine combining the shared 68000 and Z80 with ROM/SMD loading, work RAM, controller I/O, Z80 bus request/reset, VDP VRAM/CRAM/VSRAM planes/sprites/interrupt foundations, YM2612 FM/DAC/timers, SN76489 audio, SRAM persistence, and deterministic save states.
- Runnable SNES / Super Famicom development machine using the shared W65C816 with LoROM/HiROM cartridge mapping, 128 KiB WRAM, CPU/PPU I/O, controllers, vblank/NMI timing, VRAM/CGRAM/OAM register paths, Mode 0/1 BG1 rendering, SRAM persistence, and deterministic save states. General DMA plus direct/indirect HDMA table execution are implemented. The SPC700 executes through integrated S-SMP RAM/ports/timers after a firmware-free IPL upload HLE, and the S-DSP foundation now emits BRR-based stereo audio through OmniCore. Gaussian interpolation, echo/FIR and exact DSP timing, cycle-faithful HDMA timing, complete PPU modes/sprites and enhancement chips remain foundation work.
- Runnable PlayStation development machine using the shared MIPS R3000A with 2 MiB mirrored RAM, scratchpad and user-supplied 512 KiB BIOS mapping, COP0 interrupts, GPU VRAM/display plus fill/flat polygon/rectangle/copy/CPU-transfer commands, GPU block/linked-list and OTC DMA, digital-pad serial I/O, memory-card persistence, and deterministic save states. CD-ROM, SPU audio, textured GPU primitives and broad timing validation remain foundation work.

The browser stages local files into OmniCore in bounded chunks rather than first duplicating them into one temporary WASM allocation. The same resource ABI supports multiple firmware, key, disc, and storage slots for later machines. Machine-owned persistent resources have a generic ABI; cartridge RAM, including NES, Master System, Genesis, SNES, and PlayStation memory-card persistence, is automatically restored and checkpointed in IndexedDB using a content-derived game fingerprint. The current launch path still stages the complete selected file; true host-backed demand paging for very large disc/package media is a later implementation step.
