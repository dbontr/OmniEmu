...
- Reusable Sony SPC700 execution engine with all 256 opcode decode paths, direct-page/bit operations, 8/16-bit arithmetic, branches/calls/stack flow, multiply/divide/decimal adjust, and deterministic state serialization; S-SMP RAM, ports and timers are now integrated into the SNES machine, with a firmware-free IPL transfer HLE that starts uploaded SPC700 programs; S-DSP sample synthesis remains.
...
- Runnable SNES / Super Famicom development machine using the shared W65C816 with LoROM/HiROM cartridge mapping, 128 KiB WRAM, CPU/PPU I/O, controllers, vblank/NMI timing, VRAM/CGRAM/OAM register paths, Mode 0/1 BG1 rendering, SRAM persistence, and deterministic save states. General DMA plus direct/indirect HDMA table execution are implemented. The SPC700 now executes through integrated S-SMP RAM/ports/timers after a firmware-free IPL upload HLE; S-DSP sample synthesis, cycle-faithful HDMA timing, complete PPU modes/sprites and enhancement chips remain foundation work.
